...
def _merge_layer_type_mask(
    ...
    *,
    ...
    if attention_mask.dtype == torch.bool:
        external_mask = torch.full(attention_mask.shape, torch.finfo(dtype).min, dtype=dtype, device=device)
        external_mask = external_mask.masked_fill(attention_mask, 0.0)
    else:
        external_mask = attention_mask.to(dtype=dtype)
    return torch.minimum(external_mask, layer_mask)
...
class WrappedGemma4Block(_BaseDecoderLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attention_mask = _merge_layer_type_mask(
            attention_mask,
