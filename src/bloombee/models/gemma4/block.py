...
def _normalize_external_attention_mask(attention_mask: torch.Tensor, dtype: torch.dtype) -> torch.Tensor:
    ...
    if attention_mask.dtype == torch.bool:
        ...
        return torch.where(
            ...
            torch.tensor(neg_inf, dtype=dtype, device=attention_mask.device),
        )
    if attention_mask.is_floating_point() and attention_mask.dtype != dtype:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is None:
            ...
            attention_mask = _normalize_external_attention_mask(attention_mask, hidden_states.dtype)
            if self.is_sliding:
                attention_mask = torch.minimum(attention_mask, layer_type_mask)
