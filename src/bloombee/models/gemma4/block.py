...
def _to_additive_attention_mask(
    ...
    *,
    ...
    """Normalize bool/additive masks to additive scores.

    BloomBee's backend bool masks use True for visible positions. HuggingFace
    decoder layers consume additive scores where masked positions are very
    negative, so keep caller constraints while allowing stricter layer masks to
    be merged below.
    """
...
def _merge_with_layer_type_mask(
    attention_mask: torch.Tensor,
    layer_type_mask: torch.Tensor,
    *,
    dtype: torch.dtype,
    device: torch.device,
) -> torch.Tensor:
    attention_mask = _to_additive_attention_mask(
        attention_mask,
        dtype=dtype,
        device=device,
    )
    ...
    return torch.minimum(attention_mask, layer_type_mask)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is None:
            ...
            attention_mask = _merge_with_layer_type_mask(
                attention_mask,
                layer_type_mask,
                dtype=hidden_states.dtype,
                device=hidden_states.device,
