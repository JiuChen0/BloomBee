    ...
    def _spec_cache_valid_mask(
        ...
        *,
        ...
        if kv_cache_position_ids is None or is_dummy(kv_cache_position_ids):
            raise RuntimeError(
                "Speculative cache position metadata is required to expand a local tree mask "
                f"with cache_len={cache_len}"
            )
        ...
        if ids.ndim < 2 or ids.shape[0] != batch_size:
            raise RuntimeError(
                "Speculative cache position metadata batch mismatch after slicing: "
                f"got={tuple(ids.shape) if torch.is_tensor(ids) else None}, "
                f"expected_batch={batch_size}"
            )
