...
class TransformerConnectionHandler(ConnectionHandler):
    ...
    async def _push_microbatch(
        ...
        try:
            ...
            release_slot_handed_off = acquired_slot
            if is_log_channel_enabled("s2s_wire_logs"):
                logger.info(
            ...
            if acquired_slot and not release_slot_handed_off and hasattr(self, "_push_limiter"):
                measured_send_ms = (perf_counter() - push_start_time) * 1000.0
                await self._push_limiter.release(send_time_ms=measured_send_ms, success=False)
            logger.warning(
