...
def test_mixtral_saves_rotary_before_dtype_apply():
    before, after = _split_around_super_apply(
        _apply_method("src/bloombee/models/mixtral/block.py", "WrappedMixtralBlock")
    )
    assert _mentions_name(before, "rotary_buffers")
    assert _mentions_name(after, "rotary_buffers")
    assert not _calls_method(after, "float")
...
def test_falcon_saves_rotary_before_dtype_apply():
    before, after = _split_around_super_apply(
        _apply_method("src/bloombee/models/falcon/block.py", "OptimizedFalconRotaryEmbedding")
    )
    assert _mentions_name(before, "inv_freq")
    assert _mentions_name(after, "inv_freq")
    assert not _calls_method(after, "float")
